...
func _process(_delta: float) -> void:
	if is_instance_valid(dialogue_line):
		if not dialogue_label.is_typing and dialogue_line.responses.size() == 0 and not dialogue_line.has_tag("voice"):
			...
			get_node("Balloon/MarginContainer/Control/AnimationPlayer").play("progress")
		else:
...
func apply_dialogue_line() -> void:
	...
	if dialogue_line.has_tag("voice"):
		talk_sound.stream = load(dialogue_line.get_tag_value("voice"))
		talk_sound.play()
		await talk_sound.finished
